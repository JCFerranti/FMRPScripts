...
#Ribeirao Preto,1 de outubro de 2012      #
#versao 1 
###############################################################
#####################################################################
#@NAME       : relaxot2.pl
#@INPUT      : 
#@OUTPUT     : Disc_relaxot2.txt 
#@RETURNS    : 
#@DESCRIPTION: Extração dos dados de relaxometria T2 em 5 discos vertebrais.
...
#**************Resultados de T2*************************************
 ...
 open(RELATDADOS, "> Disc_T2.txt");				# Abertura de arquivo para salvar dados de ADC
 print RELATDADOS "name; T2_DL5S1 (mm²/s); ±DP(mm²/s); T2_DL4L5 (mm²/s); ±DP(mm²/s); T2_DL3L4 (mm²/s); ±DP(mm²/s); T2_DL2L3 (mm²/s); ±DP(mm²/s); T2_DL1L2 (mm²/s); ±DP(mm²/s)\n";
 $c=0;
 print "Nomes: ".$nomes."\n"; 
 while($c != $tam)
	{  
         print "Escrevendo resultados: \n"; 
	 print RELATDADOS $nomes[$c].";".$T2_DL5S1_mean[$c].";".$T2_DL5S1_DP[$c].";".$T2_DL4L5_mean[$c].";".$T2_DL4L5_DP[$c].";".$T2_DL3L4_mean[$c].";".$T2_DL3L4_DP[$c].";".$T2_DL2L3_mean[$c].";".$T2_DL2L3_DP[$c].";".$T2_DL1L2_mean[$c].";".$T2_DL1L2_DP[$c]."\n";
	 $c=$c+1;
	}
...
#************************Estimativa do T2***************************************************
 $sh->res($lbl,$PrimNome."_relaxo");  				# coregistra label com mapa adc
...
#************************Media em Disco DL5S1**********************************************************
 ...
 $T2_DL5S1_mean = substr($temp1,0,rindex($temp1," "));
 print "DL5S1_mean: ".$T2_DL5S1_mean."\n";
 $sh->rm("volume1.txt");
 push(@T2_DL5S1_mean,$T2_DL5S1_mean);
