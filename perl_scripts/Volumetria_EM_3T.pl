...
open(RELATDADOS, "> Dados_volume_classify_total_brain.txt");
    $tam =@nomes;
    print RELATDADOS "nome;CSF(cm3);GM(cm3);WM(cm3);Vol IC T(cm3);Vol enc(cm3);vol cerebelo_ponte_bulbo(cm3); Volume_lesao;BPF(%)\r";
	$c=0;
	while($c != $tam)
	{  
	   print RELATDADOS $nomes[$c].";".$volumes_liquor[$c].";".$volumes_MC[$c].";".$volumes_MB[$c].";".$volumes_intracraniano[$c].";".$volumes_encefalo[$c].";".$volumes_cerebelo[$c].";".$volumes_lesao[$c].";".$volumes_BPF[$c]."\r";
	  $c=$c+1;
	}
sub Processamento
{   chdir($NomePasta);
 ...
 $sh->res(" ".$Nomed1." ".$NomePaciente."_classified_s_lesao");
 @vectorres = glob("./*_mpr_res.mnc.gz");
 $sh->mincstats(" -volume ".$vectorres[0]." -mask ".$NomePaciente."_classified_s_lesao.mnc.gz -mask_binvalue 1 > volume1.txt");
 ...
 $sh->mincstats(" -volume ".$vectorres[0]." -mask ".$NomePaciente."_classified_s_lesao.mnc.gz -mask_binvalue 2 > volume3.txt");
 ...
 $sh->mincstats(" -volume ".$vectorres[0]." -mask ".$NomePaciente."_classified_s_lesao.mnc.gz -mask_binvalue 3 > volume4.txt");
 ...
 $volumeConv_cerebelo = $volumeConv_intracraniano - $volumeConv_encefalo;
  ...
  $BPF = (($volumeConv_MC + $volumeConv_MB)/$volumeConv_intracraniano)*100;
  print"\n\n BPF: ".$BPF."\n\n";
  push(@volumes_BPF,$BPF);

 #*Calculo do volume de lesao******************************************************************
	@vectorarquivo = glob("./*_0_flair.mnc.gz");
	$arquivo2=$vectorarquivo[0];
	$Start = index($arquivo2,"./") + 2;
	$Stop = index($arquivo2,".mnc.gz")-2;
	$Nomed2 = substr($arquivo2,$Start,$Stop);
 $sh->res(" ".$Nomed2." ".$NomePaciente."_0_lesoes");
 @vectorres2 = glob("./*_flair_res.mnc.gz");
 $sh->mincstats(" -volume ".$vectorres2[0]." -mask ".$NomePaciente."_0_lesoes.mnc.gz -mask_binvalue 1 > volume1.txt");
 $tag1 = $sh->grep(" Volume volume1.txt");
 $temp1 = substr($tag1,rindex($tag1,":")+7);
 $volume1 = substr($temp1,0,rindex($temp1," "));

 $volumelesao = $volume1/1000;
  print"\n\n volume de lesão: ". $volumelesao."\n\n";
  push(@volumes_lesao,$volumelesao);
  $sh->rm("volume1.txt");
