...
if(@nomese1)
  { $tam =@nomese1;
    print RELATERR "<b><BR> Ocorreram erros quando os arquivos e1 foram concatenados em :<BR> </b>";
	$c=0;
	while($c != $tam)
	{ print RELATERR "<b>".$nomese1[$c]."<BR></b>";
	  $c=$c+1;
	}
  }
...
if(@nomese2)
  { $tam =@nomese1;
    print RELATERR "<b><BR> Ocorreram erros quando os arquivos e2 foram concatenados em :<BR> </b>";
	$c=0;
	while($c != $tam)
	{ print RELATERR "<b>".$nomese2[$c]."<BR></b>";
	  $c=$c+1;
	}
  }
else {print RELATERR "<b><BR>NÃO ocorreram ERROS quando os arquivos foram renomeados <BR></b>";}
...
if(@nomesgd)
  { $tam =@nomesgd;
    print RELATERR "<b><BR> Ocorreram erros quando os arquivos gd foram concatenados em :<BR> </b>";
	$c=0;
	while($c != $tam)
	{ print RELATERR "<b>".$nomesgd[$c]."<BR></b>";
	  $c=$c+1;
	}
  }
else {print RELATERR "<b><BR>NÃO ocorreram ERROS quando os arquivos foram renomeados <BR></b>";}
			 ...
			 if($repetition == 0.034)
				{ $tag2 = $sh->grep(" dicom_0x0018:el_0x0021 mincheader.txt");
				  ...
				  $MT = substr($MT1,$Start,$Stop);
					...
					{
						if($repetition == 0.56 || $repetition == 0.54 || $repetition == 0.672)
						{	
...
{
	#@vectorconc = glob("./*");
	#$testeconc = @vectorconc;
	#$marcconc=0;
	#while($marcconc != $testeconc )
	#{  
	  # chdir($vectorconc[0]);
	  # Veja o gadolineo
	  @todasgd = glob("./*");
	  @Imgd = grep(/(gd)/,@todasgd);
	  $NumGado = @Imgd;
	   if($NumGado == 2)
	   {
		   print "\n Concatenando Gadolinio...\n";
		   print "\n veja: $Imgd[0] \n";
		   print "\n veja: $Imgd[1] \n";
		   #$sh->mritoself(" -close ".$Imgd[0]." ".$Imgd[1]." ".$NomePaciente."_gdTOgd.xfm");
		   #$sh->mincresample(" ".$Imgd[0]." ".$NomePaciente."_gdTOgd.mnc -like ".$Imgd[1]." -transformation ".$NomePaciente."_gdTOgd.xfm");
		   $sh->mincconcat(" ".$Imgd[0]." ".$Imgd[1]." ".$NomePaciente."_".$ano.$mes.$dia."_0_gd.mnc");
		   $errInt=$sh->echo($?);#se ainda não conseguiu concatenar
		   if($errInt==0)
           { $sh->rm("".$Imgd[0]);
		     $sh->rm("".$Imgd[1]);
			 $sh->gzip(" ".$NomePaciente."_".$ano.$mes.$dia."_0_gd.mnc");			 
		   }
		   else{push(@nomesgd,$NomePaciente."_".$ano.$mes.$dia);}

		   #$sh->rm("".$NomePaciente."_gdTOgd.xfm");
		   #$sh->rm("".$NomePaciente."_gdTOgd.mnc");
		   
	   }
	   #Veja a e1  
	   @todase1 = glob("./*");
	   @Ime1 = grep(/(e1)/,@todase1);
	   $Nume1 = @Ime1;
	   if($Nume1 == 2)
	   {
		   print "\n Concatenando E1...\n";
		   print "\n veja: $Ime1[0] \n";
		   print "\n veja: $Ime1[1] \n";
		   #$sh->mritoself(" -close ".$Ime1[0]." ".$Ime1[1]." ".$NomePaciente."_e1TOe1.xfm");
		   #$sh->mincresample(" ".$Ime1[0]." ".$NomePaciente."_e1TOe1.mnc -like ".$Ime1[1]." -transformation ".$NomePaciente."_e1TOe1.xfm");
		   $sh->mincconcat(" ".$Ime1[0]." ".$Ime1[1]." ".$NomePaciente."_".$ano.$mes.$dia."_0_e1.mnc");
		   $errInt1=$sh->echo($?);#se ainda não conseguiu concatenar
			if($errInt1==0)
           {  $sh->rm("".$Ime1[0]);
		      $sh->rm("".$Ime1[1]);		   
		      $sh->gzip(" ".$NomePaciente."_".$ano.$mes.$dia."_0_e1.mnc"); 
		   }
		   else{push(@nomese1,$NomePaciente."_".$ano.$mes.$dia);}
		  
		   #$sh->rm("".$NomePaciente."_e1TOe1.xfm");
		   #$sh->rm("".$NomePaciente."_e1TOe1.mnc");
		   
	   }
	   #Veja a e12 
	   @todase2 = glob("./*");
	   @Ime2 = grep(/(e2)/,@todase2);
	   $Nume2 = @Ime2;
	   if($Nume2 == 2)
	   {
		   print "\n Concatenando E2...\n";
		   print "\n $Ime2[0] \n";
		   print "\n $Ime2[1] \n";
		  # $sh->mritoself(" -close ".$Ime2[0]." ".$Ime2[1]." ".$NomePaciente."_e2TOe2.xfm");
		  # $sh->mincresample(" ".$Ime2[0]." ".$NomePaciente."_e2TOe2.mnc -like ".$Ime2[1]." -transformation ".$NomePaciente."_e2TOe2.xfm");
		   $sh->mincconcat(" ".$Ime2[0]." ".$Ime2[1]." ".$NomePaciente."_".$ano.$mes.$dia."_0_e2.mnc");
		   $errInt2=$sh->echo($?);#se ainda não conseguiu concatenar
		   if($errInt2==0)
           {  $sh->rm("".$Ime2[0]);
		      $sh->rm("".$Ime2[1]);		   
		      $sh->gzip(" ".$NomePaciente."_".$ano.$mes.$dia."_0_e2.mnc"); 
		   }
		   else{push(@nomese2,$NomePaciente."_".$ano.$mes.$dia);}
		   
		   #$sh->rm("".$NomePaciente."_e2TOe2.xfm");
		   #$sh->rm("".$NomePaciente."_e2TOe2.mnc");
