...
open(RELATDADOS, "> Data_Fat_Percentage.txt");
    $tam =@nomes;
	print RELATDADOS "name; Figado_Media FAT(%); Figado_DesvioP FAT (%)\n";
	$c=0;
	while($c != $tam)
	{  
	 print RELATDADOS $nomes[$c].";".$media_fat_figado[$c].";".$desvpad_fat_figado[$c]."\n";
	 $c=$c+1;
	}

 open(RELATDADOS, "> Data_Fat_Volume.txt");
    $tam =@nomes;
	print RELATDADOS "name; Subcutaneous FAT(cm³); Intraperitoneal FAT (cm³); Figado FAT(cm³); Total FAT(cm³)\n";
	$c=0;
	while($c != $tam)
	{  
	 print RELATDADOS $nomes[$c].";".$volumes_subcutanea[$c].";".$volumes_peritonial[$c].";".$volumes_figado[$c].";".$volumes_total[$c]."\n";
	 $c=$c+1;
	}
...
$sh->res(" ".$lbl." fat_map ");
 ...
 $sh->mincstats(" -stddesv fat_map.mnc.gz -mask ".$lbl."_res.mnc.gz -mask_binvalue 3 > volume1.txt");
 $tag1 = $sh->grep(" Mean volume1.txt");
 $temp1 = substr($tag1,rindex($tag1,":")+15);
