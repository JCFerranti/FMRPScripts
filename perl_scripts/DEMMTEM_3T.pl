...
#Subtsancia branca
 ...
 $sh->rm(" ".$PrimNome."_".$ano.$mes.$dia."_0_gray_matter_res.mnc.gz");
  ...
  $sh->rm(" *_res.mnc.gz"); 
   ...
   #lesoes_t1
 $sh->res(" ".$PrimNome."_".$ano.$mes.$dia."_0_lesoes ".$PrimNome."_relaxo");
 $sh->mincstats(" -mean ".$PrimNome."_relaxo.mnc.gz -mask ".$PrimNome."_".$ano.$mes.$dia."_0_lesoes_res.mnc.gz -mask_binvalue 1 > volume1.txt");
 $tag1 = $sh->grep(" Mean volume1.txt");
 $temp1 = substr($tag1,rindex($tag1,":")+15);
 $media_relaxo_Lt1 = substr($temp1,0,rindex($temp1," "));
 $sh->rm("volume1.txt");
 
 $sh->mincstats(" -stddev  ".$PrimNome."_relaxo.mnc.gz -mask ".$PrimNome."_".$ano.$mes.$dia."_0_lesoes_res.mnc.gz -mask_binvalue 1 > volume1.txt");
 $tag1 = $sh->grep(" Stddev volume1.txt");
 $temp1 = substr($tag1,rindex($tag1,":")+13);
