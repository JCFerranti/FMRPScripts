			 ...
			 if($repetition == 0.034)
				{ $tag2 = $sh->grep(" dicom_0x0018:el_0x0021 mincheader.txt");
				  ...
				  $MT = substr($MT1,$Start,$Stop);
					...
					{if($repetition == 0.)
					  { print"\n\n\ veja o nome do arquivo para difusão:".$vector4[0]."\n\n";
									$start2 = index($vector4[0],"./") + 2;
									...
									print"\n\n difusão:".$file_dif."\n\n";
									$sh->mv(" ".$mprname." ".$NomePaciente."_".$ano.$mes.$dia."_".$idif."_dif.mnc");
								    $sh->gzip(" ".$NomePaciente."_".$ano.$mes.$dia."_".$idif."_dif.mnc");
									$idif = $idif + 1;
					   }else					
