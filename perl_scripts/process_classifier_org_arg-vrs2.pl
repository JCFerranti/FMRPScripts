...
# &ClassificaImagem;
 ...
 $PrimNome = $NomePaciente;
 print "\n\n $PrimNome \n\n";
 
#******************************Renomeia os arquivos****************************************************
 #$sh->mv(" $vector3[0] ".$PrimNome."$ARGV[0]_d1.mnc.gz");
 #$sh->mv(" $vector3[1] ".$PrimNome."$ARGV[0]_d2.mnc.gz");
 #shift(@vector3);
 #shift(@vector3);
 ...
 #$sh->mincconcat(" $Im1[0] $Im1[1] ".$PrimNome."$ARGV[0]_e1.mnc");
 #$err1=$sh->echo($?);
 #**********************Correção de erro ******************************************
  #if($err1!=0)
 #{ 
  ...
  # $sh->mritoself(" -close ".$Im1[0]." ".$Im1[1]." ".$PrimNome."_e1TOe1.xfm");
   ...
   #$sh->mincconcat(" ".$PrimNome."_e1TOe1.mnc $Im1[1] ".$PrimNome."$ARGV[0]_e1.mnc");
 ...
 #}
 #*********************************************************************************
 #$sh->gzip(" ".$PrimNome."$ARGV[0]_e1.mnc");
 #$sh->rm(" $Im1[0]");
 #$sh->rm(" $Im1[1]");
 #**************************************************************************************************************************************************************
 #@Im2 = grep(/(e2)/,@vector3);
 #print "\n veja isso : $Im2[0]\n\n";
 #print "\n veja isso : $Im2[1]\n\n";
 #$sh->mincconcat(" $Im2[0] $Im2[1] ".$PrimNome."$ARGV[0]_e2.mnc");
 #$err2=$sh->echo($?);
 #**********************Correção de erro ******************************************
 # if($err2!=0)
 { 
   ...
   #$sh->mincconcat(" ".$PrimNome."_e2TOe2.mnc $Im2[1] ".$PrimNome."$ARGV[0]_e2.mnc");
   #$errInt=$sh->echo($?);#se ainda não conseguiu concatenar
   #if($errInt==0)
   #{$err1=0;}
 ...
 #}
 #*********************************************************************************
 #$sh->gzip(" ".$PrimNome."$ARGV[0]_e2.mnc");
 #shift(@vector3);
 #shift(@vector3);
 #shift(@vector3);
 #shift(@vector3); 
 ...
 #print "\n veja isso : $vector3[0]\n\n";
 #print "\n veja isso : $vector3[1]\n\n";
 #$sh->mincconcat(" $vector3[0] $vector3[1] ".$PrimNome."$ARGV[0]_gd.mnc");
 #$err3=$sh->echo($?);
 #if($err3!=0)
 #{ 
   #fazendo a transformtion
   ...
   #$sh->mincconcat(" ".$PrimNome."_gdTOgd.mnc $Im2[1] ".$PrimNome."$ARGV[0]_gd.mnc");
   #$errInt=$sh->echo($?);#se ainda não conseguiu concatenar
   #if($errInt==0)
   #{$err1=0;}
 ...
 #}
 #$sh->gzip(" ".$PrimNome."$ARGV[0]_gd.mnc");
